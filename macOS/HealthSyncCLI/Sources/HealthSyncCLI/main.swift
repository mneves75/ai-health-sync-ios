...
// MARK: - FIT Import Command (with Suunto-specific fields)
...
extension HealthSyncCLI {
    static func importFIT(args: [String]) throws {
        ...
        let isSuunto = fitFile.fileId?.manufacturer == 23

        if let outputDir {
            try writeFITToDirectory(fitFile: fitFile, fitPath: fitPath, outputDir: outputDir, format: format, isSuunto: isSuunto)
        } else {
            printFITSummary(fitFile: fitFile, format: format, isSuunto: isSuunto)
        }
    }

    private static func writeFITToDirectory(fitFile: FITFile, fitPath: String, outputDir: String, format: String, isSuunto: Bool) throws {
        ...
        do {
            ...
            if format == "json" {
                ...
                try fitSessionsCSV(fitFile.sessions, suunto: isSuunto)
                ...
                try fitRecordsCSV(fitFile.records, suunto: isSuunto)
    ...
    private static func printFITSummary(fitFile: FITFile, format: String, isSuunto: Bool) {
        if format == "json" {
            let encoder = JSONEncoder()
            encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
            encoder.dateEncodingStrategy = .iso8601
            ...
            if isSuunto { print("# Suunto device detected") }
            print("=== Sessions (\(fitFile.sessions.count)) ===")
            print(fitSessionsCSV(fitFile.sessions, suunto: isSuunto))
            if !fitFile.laps.isEmpty {
                print("\n=== Laps (\(fitFile.laps.count)) ===")
                print(fitLapsCSV(fitFile.laps))
            }
            print("\n=== Records (\(fitFile.records.count)) ===")
            print(fitRecordsCSV(fitFile.records, suunto: isSuunto))
        }
    }
    ...
    private static func fitSessionsCSV(_ sessions: [FITSession], suunto: Bool) -> String {
        ...
        if suunto { header += ";epocMl;recoveryTimeHours;aerobicTE;anaerobicTE" }
        var lines = [header]
        for s in sessions {
            var row = [
            ...
            if suunto {
                row += [
                    s.suuntoEPOCml.map { String(format: "%.1f", $0) } ?? "",
                    s.suuntoRecoveryTimeHours.map { String($0) } ?? "",
                    s.suuntoAerobicTE.map { String(format: "%.1f", $0) } ?? "",
                    s.suuntoAnaerobicTE.map { String(format: "%.1f", $0) } ?? ""
                ]
            }
    ...
    private static func fitRecordsCSV(_ records: [FITRecord], suunto: Bool) -> String {
        var header = "timestamp;lat;lon;altitudeM;heartRateBPM;cadenceRPM;speedMps;powerW;tempC;distanceM"
        if suunto { header += ";fusedSpeedMps" }
        var lines = [header]
        for r in records {
            var row = [
            ...
            if suunto { row.append(r.suuntoFusedSpeedMps.map { String(format: "%.3f", $0) } ?? "") }
            lines.append(row.joined(separator: ";"))
        }
        return lines.joined(separator: "\n")
    }
...
private struct FITSessionJSON: Encodable {
    ...
    let suuntoEPOCml: Double?
    let suuntoRecoveryTimeHours: UInt16?
    let suuntoAerobicTE: Double?
    let suuntoAnaerobicTE: Double?
    ...
    init(_ s: FITSession) {
        ...
        suuntoEPOCml = s.suuntoEPOCml; suuntoRecoveryTimeHours = s.suuntoRecoveryTimeHours
        suuntoAerobicTE = s.suuntoAerobicTE; suuntoAnaerobicTE = s.suuntoAnaerobicTE
    }
}
...
private struct FITLapJSON: Encodable {
    ...
    init(_ l: FITLap) {
        ...
        totalElapsedSeconds = l.totalElapsedSeconds; totalTimerSeconds = l.totalTimerSeconds
        totalDistanceMeters = l.totalDistanceMeters; totalCalories = l.totalCalories
        avgSpeedMetersPerSec = l.avgSpeedMetersPerSec; maxSpeedMetersPerSec = l.maxSpeedMetersPerSec
        avgHeartRateBPM = l.avgHeartRateBPM; maxHeartRateBPM = l.maxHeartRateBPM
        totalAscent = l.totalAscent; totalDescent = l.totalDescent
    }
}
...
private struct FITRecordJSON: Encodable {
    ...
    init(_ r: FITRecord) {
        ...
        positionLatDegrees = r.positionLatDegrees; positionLonDegrees = r.positionLonDegrees
        altitudeMeters = r.altitudeMeters; heartRateBPM = r.heartRateBPM
        cadenceRPM = r.cadenceRPM; speedMetersPerSec = r.speedMetersPerSec
        powerWatts = r.powerWatts; temperatureCelsius = r.temperatureCelsius
        distanceMeters = r.distanceMeters; suuntoFusedSpeedMps = r.suuntoFusedSpeedMps
    }
}
