...
struct FITFile: Sendable {
    ...
    // Garmin HRV message (mesg_num 78) — nil when not present in file
    let garminHRV: GarminHRVData?
}

/// R-R intervals collected from Garmin HRV messages (mesg_num 78).
/// Values are uint16 in units of 1/1000 second (milliseconds).
struct GarminHRVData: Sendable {
...
struct FITSession: Sendable {
    ...
    // Garmin-specific fields (nil for non-Garmin files or older firmware).
    // Field numbers from the official ANT+ FIT SDK profile (version 21.x).
    let garminTrainingStressScore: Double?         // field 78, uint16 ÷ 10 (TSS)
    let garminIntensityFactor: Double?             // field 79, uint16 ÷ 1000 (IF)
    let garminPerformanceCondition: Int8?          // field 168, sint8
    let garminThresholdPower: UInt16?              // field 203, uint16, watts
    // Garmin running dynamics session averages
    let garminAvgVerticalOscillationMm: Double?    // field 70, uint16 ÷ 5
    let garminAvgStanceTimeMs: Double?             // field 69, uint16 ÷ 10
    let garminAvgStepLengthMm: Double?             // field 129, uint16, raw mm
}
...
struct FITRecord: Sendable {
    ...
    // Garmin running dynamics per-second metrics (nil for non-running or older devices)
    let garminVerticalOscillationMm: Double?    // field 53, uint16 ÷ 5
    let garminStanceTimePercent: Double?         // field 54, uint16 ÷ 100
    let garminStanceTimeMs: Double?              // field 55, uint16 ÷ 10
    let garminVerticalRatio: Double?             // field 62, uint16 ÷ 100
    let garminStanceTimeBalancePercent: Double?  // field 63, uint16 ÷ 100
    let garminStepLengthMm: UInt16?              // field 64, uint16, raw mm
}
...
struct FITParser {
    ...
    func parse() throws -> FITFile {
        ...
        var allRRIntervalsSeconds: [Double] = []
        ...
        while reader.offset < dataEnd {
            ...
            if isCompressedTimestamp {
                ...
                if isDef {
                    ...
                    case FITGlobalMesgNum.hrv.rawValue:
                        decodeHRVMessage(rawFields, into: &allRRIntervalsSeconds)
                    default:
                        break
                    }
                }
        ...
        let garminHRV: GarminHRVData? = allRRIntervalsSeconds.isEmpty ? nil : GarminHRVData(
            rrIntervalsSeconds: allRRIntervalsSeconds,
            sdnnMilliseconds: computeSDNN(allRRIntervalsSeconds),
            rmssdMilliseconds: computeRMSSD(allRRIntervalsSeconds)
        )

        return FITFile(fileId: fileId, sessions: sessions, laps: laps, records: records, garminHRV: garminHRV)
    }
    ...
    private func decodeSession(_ fields: [RawField]) -> FITSession? {
        ...
        var garminTSS: Double?
        var garminIF: Double?
        var garminPerfCond: Int8?
        var garminThreshPower: UInt16?
        var garminAvgVO: Double?
        var garminAvgST: Double?
        var garminAvgStepLen: Double?
        ...
        for f in fields {
            ...
            // Garmin-specific session fields
            case 69:
                let v = readUInt16(f.bytes, bigEndian: false)
                if v != 0xFFFF { garminAvgST = Double(v) / 10.0 }
            case 70:
                let v = readUInt16(f.bytes, bigEndian: false)
                if v != 0xFFFF { garminAvgVO = Double(v) / 5.0 }
            case 78:
                let v = readUInt16(f.bytes, bigEndian: false)
                if v != 0xFFFF { garminTSS = Double(v) / 10.0 }
            case 79:
                let v = readUInt16(f.bytes, bigEndian: false)
                if v != 0xFFFF { garminIF = Double(v) / 1000.0 }
            case 129:
                let v = readUInt16(f.bytes, bigEndian: false)
                if v != 0xFFFF { garminAvgStepLen = Double(v) }
            case 168:
                if !f.bytes.isEmpty { garminPerfCond = Int8(bitPattern: f.bytes[0]) }
            case 203:
                let v = readUInt16(f.bytes, bigEndian: false)
                if v != 0xFFFF { garminThreshPower = v }
            default: break
            }
        }
        ...
        return FITSession(
            ...
            garminTrainingStressScore: garminTSS, garminIntensityFactor: garminIF,
            garminPerformanceCondition: garminPerfCond, garminThresholdPower: garminThreshPower,
            garminAvgVerticalOscillationMm: garminAvgVO, garminAvgStanceTimeMs: garminAvgST,
            garminAvgStepLengthMm: garminAvgStepLen
        )
    }
    ...
    private func decodeRecord(_ fields: [RawField], timestamp: UInt32) -> FITRecord? {
        ...
        var distance: Double?
        var garminVO: Double?
        var garminSTpct: Double?
        var garminSTms: Double?
        var garminVR: Double?
        var garminSTbal: Double?
        var garminStepLen: UInt16?
        ...
        for f in fields {
            ...
            case 53:
                if f.bytes.count == 2 {
                    let v = readUInt16(f.bytes, bigEndian: false)
                    if v != 0xFFFF { garminVO = Double(v) / 5.0 }
                }
            case 54:
                if f.bytes.count == 2 {
                    let v = readUInt16(f.bytes, bigEndian: false)
                    if v != 0xFFFF { garminSTpct = Double(v) / 100.0 }
                }
            case 55:
                if f.bytes.count == 2 {
                    let v = readUInt16(f.bytes, bigEndian: false)
                    if v != 0xFFFF { garminSTms = Double(v) / 10.0 }
                }
            case 62:
                if f.bytes.count == 2 {
                    let v = readUInt16(f.bytes, bigEndian: false)
                    if v != 0xFFFF { garminVR = Double(v) / 100.0 }
                }
            case 63:
                if f.bytes.count == 2 {
                    let v = readUInt16(f.bytes, bigEndian: false)
                    if v != 0xFFFF { garminSTbal = Double(v) / 100.0 }
                }
            case 64:
                if f.bytes.count == 2 {
                    let v = readUInt16(f.bytes, bigEndian: false)
                    if v != 0xFFFF { garminStepLen = v }
                }
            default: break
            }
        }
        ...
        return FITRecord(
            ...
            garminVerticalOscillationMm: garminVO,
            garminStanceTimePercent: garminSTpct,
            garminStanceTimeMs: garminSTms,
            garminVerticalRatio: garminVR,
            garminStanceTimeBalancePercent: garminSTbal,
            garminStepLengthMm: garminStepLen
        )
    }

    private func decodeHRVMessage(_ fields: [RawField], into accumulator: inout [Double]) {
        // HRV mesg (mesg_num 78): field 0 contains an array of uint16 R-R intervals
        // Each value is in units of 1/1000 s (milliseconds). 0xFFFF = sentinel/end.
        for f in fields where f.defNum == 0 {
            var idx = 0
            while idx + 1 < f.bytes.count {
                let v = UInt16(f.bytes[idx]) | (UInt16(f.bytes[idx + 1]) << 8)
                if v == 0xFFFF { break }
                accumulator.append(Double(v) / 1000.0)
                idx += 2
            }
        }
    }

    private func computeSDNN(_ rr: [Double]) -> Double? {
        guard rr.count > 1 else { return nil }
        let mean = rr.reduce(0, +) / Double(rr.count)
        let variance = rr.map { ($0 - mean) * ($0 - mean) }.reduce(0, +) / Double(rr.count)
        return sqrt(variance) * 1000
    }

    private func computeRMSSD(_ rr: [Double]) -> Double? {
        guard rr.count > 1 else { return nil }
        let squaredDiffs = zip(rr, rr.dropFirst()).map { ($1 - $0) * ($1 - $0) }
